"""
FUNCIONAL
VARIANTE "REVERSO" de llenar_actas_sigi.py: hace exactamente lo mismo,
pero recorre las PÁGINAS de la grilla en orden inverso -- de la última a
la primera. Dentro de cada página, las filas se procesan en el mismo
orden de siempre (arriba hacia abajo).
...
(commit inmediato) y saca ese registro de la lista de pendientes.
  ...
  2.1) A diferencia del script original, ANTES de leer ninguna fila avanza
     con "Siguiente" hasta la ÚLTIMA página (ver _ir_a_ultima_pagina).
  3) Recorre página por página EN REVERSA (de la última a la primera, con
     el botón "Anterior" en vez de "Siguiente"), fila por fila en cada
     una: hace click en el ojito ("Ver"), entra a la pestaña "Actas", lee
     el primer "Número acta" (formato "351.937") y lo compara
     (normalizado) contra los pendientes.
  ...
  5) Corta apenas no quedan pendientes o se llega a la página 1.
...
Pensado para correrse EN PARALELO con llenar_actas_sigi.py (uno arranca
del principio, el otro del final) contra la misma DB: cada match hace
commit inmediato y saca el registro de "pendientes", así que dos
corridas simultáneas no deberían pisarse -- en el peor caso, ambas leen
el mismo registro como pendiente antes de que la otra lo guarde y
terminan escribiéndolo dos veces (incidente menor, no corrompe datos:
`pendientes` se arma una sola vez por corrida a partir del estado de la
DB en ese momento, no se refresca fila a fila).
"""
import argparse
import asyncio
import re
...
async def _leer_primer_numero_acta(page) -> Optional[str]:
    ...
    Método principal, confirmado por inspección real del HTML: la fila
    'Datos del acta' es un <div class="flex gap-3 py-2 text-sm"> con dos
    <span> -- el label ('Número acta') y el valor
    (<span class="text-gray-900 flex-1 break-words">) como hermano
    inmediato. Se lee ese valor directo, sin depender de que el número
    tenga formato con puntos (ej. '351.937') -- ANTES se extraía con un
    regex de formato fijo sobre TODO el texto de la página
    (reglas_sigi.extraer_numero_acta_de_texto), lo que dejaba afuera
    cualquier acta con un número corto sin puntos (ej. '3').

    Fallback: si por lo que sea no aparece ese label (ej. cambió el
    texto o el layout), se cae al método viejo -- regex de formato con
    puntos sobre el texto completo de la página.
    ...
    log("DEBUG-ACTA", f"No se encontró el label 'Número acta' (o vino vacío) tras {intentos} "
                       f"intentos -- caigo al método viejo (regex de formato con puntos "
                       f"sobre todo el texto de la página)")
    ...
    log("DEBUG-ACTA", f"Pestaña 'Actas' abierta pero no se encontró número de acta tras "
                       f"{intentos} intentos (ni por label ni por regex viejo). Primeros 500 "
                       f"chars del texto leído:\n{texto[:500]}")
    return None
...
SELECTOR_SPAN_ANTERIOR_MOBILE = "span.sm\\:hidden"


async def _boton_anterior(page):
    """
    Ubica el botón 'Anterior'.

    Método principal, confirmado por inspección real del HTML: 'Anterior'
    vive en el mismo contenedor que el input 'Página [n] de N'
    (aria-label='Ir a la página', ver _contenedor_paginador_pagina) junto
    con 'Siguiente' -- se busca por texto DENTRO de ese contenedor. No
    importa si el texto real está partido en <span> según breakpoint
    (mobile/desktop, mismo patrón que se documentó para 'Siguiente'),
    porque el selector `has_text` matchea el textContent completo del
    <button> sin importar si algún <span> interno está oculto por CSS.

    Fallback (por si el input no aparece o el contenedor no tiene
    exactamente un botón 'Anterior'): el guess viejo, simétrico al de
    'Siguiente' -- span mobile '<' o el botón inmediatamente anterior al
    texto de paginación.
    """
    contenedor = await _contenedor_paginador_pagina(page)
    if contenedor is not None:
        boton = contenedor.locator("button", has_text="Anterior")
        total_boton = await boton.count()
        if total_boton == 1:
            log("DEBUG-PAGINADO", "Botón 'Anterior' ubicado por el contenedor confirmado del input 'Página'")
            return boton
        log("DEBUG-PAGINADO", f"⚠️ {total_boton} candidato(s) a 'Anterior' dentro del contenedor "
                               f"del input 'Página' (esperaba 1) -- caigo al selector viejo")

    span_mobile = page.locator(SELECTOR_SPAN_ANTERIOR_MOBILE, has_text=re.compile(r"^\s*<\s*$"))
    total_spans = await span_mobile.count()

    if total_spans == 1:
        boton = span_mobile.locator("xpath=ancestor::button[1]")
        if await boton.count() == 1:
            log("DEBUG-PAGINADO", "Botón 'Anterior' ubicado por el span mobile ('<' / sm:hidden)")
            return boton
        log("DEBUG-PAGINADO", f"⚠️ El span '<' no tiene un <button> ancestro directo "
                               f"(count={await boton.count()}) -- caigo al selector por posición")
    elif total_spans > 1:
        log("DEBUG-PAGINADO", f"⚠️ Hay {total_spans} spans '<' con clase sm:hidden en la página "
                               f"(ambiguo, no sé cuál es 'Anterior') -- caigo al selector por posición")
    else:
        log("DEBUG-PAGINADO", "No se encontró ningún span '<' con clase sm:hidden -- "
                               "caigo al selector por posición")

    return page.get_by_text(PATRON_PAGINA_ACTUAL).first.locator("xpath=preceding::button[1]")


async def _hay_pagina_anterior(page, intentos: int = 5, espera_ms: int = 500) -> bool:
    """Análogo a _hay_pagina_siguiente pero para el botón 'Anterior' --
    misma espera de paginador estable antes de mirar el botón (ver
    _esperar_paginador_estable)."""
    await _esperar_paginador_estable(page)

    boton_anterior = await _boton_anterior(page)
    total = await boton_anterior.count()
    if total == 0:
        log("DEBUG-PAGINADO", "No se encontró el botón 'Anterior' (ni por posición)")
        return False

    log("DEBUG-PAGINADO", f"Botón candidato encontrado: {await _describir_elemento(boton_anterior)}")

    for intento in range(1, intentos + 1):
        habilitado = await boton_anterior.is_enabled()
        if habilitado:
            log("DEBUG-PAGINADO", f"Botón 'Anterior' habilitado (intento {intento})")
            return True
        if intento < intentos:
            await page.wait_for_timeout(espera_ms)

    log("DEBUG-PAGINADO", f"Botón 'Anterior' encontrado pero deshabilitado tras {intentos} intentos "
                           f"-- asumo que ya estamos en la página 1")
    return False


async def _ir_a_pagina_anterior(page):
    boton_anterior = await _boton_anterior(page)
    pagina_antes = await _leer_numero_pagina_actual(page)
    log("DEBUG-PAGINADO", f"Voy a clickear: {await _describir_elemento(boton_anterior)}")
    await boton_anterior.click()
    await _esperar_red(page)
    ...
    if pagina_antes and pagina_despues and pagina_despues == pagina_antes:
        log("DEBUG-PAGINADO", f"⚠️ Hice click pero seguimos en la página {pagina_despues} "
                               ...
                               f"NO es el de 'Anterior' real")
    else:
        log("DEBUG-PAGINADO", f"Página antes del click: {pagina_antes} -> después: {pagina_despues}")


PATRON_TOTAL_PAGINAS = re.compile(r"de\s+([\d.,]+)")
# Confirmado por inspección real del HTML: el input de "Página [n] de N"
# tiene este aria-label -- selector directo y estable, no hace falta
# adivinar por proximidad de texto.
SELECTOR_INPUT_PAGINA = "input[aria-label='Ir a la página']"


async def _contenedor_paginador_pagina(page):
    """Contenedor (el <div> con display:flex visto en la captura real)
    que agrupa 'Anterior', el <label> de 'Página [n] de N' y 'Siguiente'.
    Se sube un nivel desde el <input> (aria-label='Ir a la página') --
    ese input está anidado en un <label>, y el <div> que lo contiene es
    el que también tiene los botones 'Anterior'/'Siguiente' como
    hermanos del <label> (mismo layout que la captura: 'Anterior |
    Página [n] de N | Siguiente')."""
    input_pagina = page.locator(SELECTOR_INPUT_PAGINA)
    if await input_pagina.count() == 0:
        return None
    return input_pagina.locator("xpath=ancestor::label[1]/..")


async def _ubicar_input_pagina(page):
    """Ubica el <input> de 'Página [n] de N' -- confirmado por
    inspección real del HTML: aria-label='Ir a la página'."""
    input_pagina = page.locator(SELECTOR_INPUT_PAGINA)
    if await input_pagina.count() == 0:
        log("DEBUG-PAGINADO", "⚠️ No se encontró el input[aria-label='Ir a la página']")
        return None
    return input_pagina


async def _leer_total_paginas(page) -> Optional[int]:
    """Lee el N de 'Página [n] de N' -- confirmado por inspección real:
    el <label> que envuelve el input (aria-label='Ir a la página') tiene
    un <span>de <span>3518</span></span> hermano. El value del input NO
    aparece en innerText (es el `value` de un <input>, no un nodo de
    texto), así que leer el <label> completo da directamente
    '...de 3518', que alcanza para el regex."""
    input_pagina = page.locator(SELECTOR_INPUT_PAGINA)
    if await input_pagina.count() == 0:
        return None
    ...
    try:
        ...
        log("DEBUG-PAGINADO", f"⚠️ No se pudo leer el <label> de 'Página X de N': {exc}")
        return None
    match = PATRON_TOTAL_PAGINAS.search(texto)
    if not match:
        log("DEBUG-PAGINADO", f"⚠️ No matcheó '{PATRON_TOTAL_PAGINAS.pattern}' en el texto "
                               f"del <label>: {texto!r}")
        return None
    return int(match.group(1).replace(".", "").replace(",", ""))


async def _saltar_a_pagina(page, numero_pagina: int) -> bool:
    """Escribe `numero_pagina` en el input 'Página' y aprieta Enter para
    saltar directo (ver captura real del paginador), en vez de clickear
    'Siguiente' una por una. Devuelve False (sin reventar nada) si el
    input no aparece o falla el fill/Enter -- el llamador cae al método
    viejo (clickear 'Siguiente' hasta el final) como red de seguridad."""
    input_pagina = await _ubicar_input_pagina(page)
    if input_pagina is None:
        log("DEBUG-PAGINADO", "⚠️ No se encontró el <input> de 'Página' para saltar directo")
        return False
    try:
        await input_pagina.fill(str(numero_pagina))
        await input_pagina.press("Enter")
    except Exception as exc:
        log("DEBUG-PAGINADO", f"⚠️ No se pudo escribir/enterar en el input de página: {exc}")
        return False
    await _esperar_red(page)
    await _esperar_tabla_lista(page)
    return True
...
async def _ir_a_ultima_pagina(page) -> int:
    """Paso previo al recorrido en reversa: llega a la última página.

    Primero intenta el atajo -- leer el total de páginas del input
    'Página [n] de N' (ver captura real) y saltar directo escribiendo ese
    número + Enter, en vez de clickear 'Siguiente' miles de veces. Se
    verifica el resultado leyendo la página actual con el método de
    siempre (_leer_numero_pagina_actual, basado en 'Mostrando A a B de C
    resultados', que es el que ya está confirmado y probado).

    Si el atajo falla por lo que sea (no se encontró el input, no se
    pudo leer el total, el salto no navegó como se esperaba), cae al
    método anterior: clickear 'Siguiente' hasta que se deshabilite."""
    total_paginas = await _leer_total_paginas(page)
    pagina_actual = await _leer_numero_pagina_actual(page) or 1

    if total_paginas:
        log("PAGINA", f"Recorrido en REVERSO: {total_paginas} página(s) en total -- "
                      f"salto directo a la última escribiendo el número y Enter...")
        if await _saltar_a_pagina(page, total_paginas):
            pagina_verificada = await _leer_numero_pagina_actual(page)
            if pagina_verificada == total_paginas:
                log("PAGINA", f"📍 Última página alcanzada de un salto: {pagina_verificada}.")
                return pagina_verificada
            log("PAGINA", f"⚠️ Salté a la página pero quedé en {pagina_verificada!r} en vez de "
                          f"{total_paginas} -- termino de acercarme con 'Siguiente'...")
            pagina_actual = pagina_verificada or pagina_actual
        else:
            log("PAGINA", "⚠️ El salto directo falló -- recorro con 'Siguiente' (método lento)")
    else:
        log("PAGINA", "⚠️ No pude leer el total de páginas para saltar directo -- "
                      "recorro con 'Siguiente' hasta la última (método lento)")

    while await _hay_pagina_siguiente(page):
        await _ir_a_pagina_siguiente(page)
        pagina_actual += 1
    log("PAGINA", f"📍 Última página alcanzada: {pagina_actual}. Arranca el recorrido hacia atrás.")
...
async def _asegurar_pagina(page, pagina_objetivo: int):
    ...
    if await _saltar_a_pagina(page, pagina_objetivo):
        verificada = await _leer_numero_pagina_actual(page)
        if verificada == pagina_objetivo:
            log("PAGINA", f"✅ Recuperada de un salto directo, quedamos en página {verificada}")
            return
        log("PAGINA", f"⚠️ El salto directo dejó la página en {verificada!r} en vez de "
                      f"{pagina_objetivo} -- termino de acercarme con 'Siguiente'...")
...
async def _procesar_todas_las_paginas(db: Session, page, pendientes: dict, commit: bool,
    ...
    """
    Recorre la tabla completa de SIGI, fila por fila, EN REVERSA: primero
    salta a la última página (_ir_a_ultima_pagina) y después va retrocediendo
    página por página con el botón 'Anterior' hasta la página 1 (o hasta que
    no queden pendientes). Dentro de cada página, las filas se procesan en
    el mismo orden de siempre (arriba hacia abajo).
    ...
    pagina_actual = await _ir_a_ultima_pagina(page)
    encontrados = 0
    ...
    while pendientes:
        ...
        if pagina_actual <= 1:
            ...
            break
        if not await _hay_pagina_anterior(page):
            log("PAGINA", f"No hay botón 'Anterior' habilitado (estábamos en la página "
                          f"{pagina_actual}) -- fin del recorrido en reversa.")
            break
        await _ir_a_pagina_anterior(page)
        pagina_actual -= 1
...
async def ejecutar_alta(db: Session, page, commit: bool = True) -> dict:
    """
    Núcleo del paso de alta (variante REVERSO), pensado para reutilizarse
    con una sesión de navegador ya abierta (ver
    backend/app/pasos/procesar_actas_sigi.py). Recorre TODA la tabla de
    SIGI una sola vez, fila por fila, de la ÚLTIMA página a la primera,
    buscando coincidencias contra las actas pendientes de la DB.
