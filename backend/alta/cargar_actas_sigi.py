...
que la acta ya esté para saltear la fila).
  ...
  1) Junta de la DB el conjunto de todas las actas ya conocidas (ver
     reglas_sigi.todas_las_actas_conocidas).
  ...
  3) Recorre página por página, fila por fila: hace click en el ojito
     ("Ver"), entra a la pestaña "Actas" y lee ahí el Nº de acta, la
     patente, la dirección y la fecha/hora del acta.
  4) Si el acta YA está en la base -> se saltea la fila, no se toca nada.
     Si NO está -> se da de alta un registro nuevo con expediente (de la
     grilla), acta/patente/dirección/fecha (del detalle) y estado_sigi
     según lo que muestra la columna ESTADO de la grilla. estado_semyt
     queda siempre en 'Eliminada' (ver reglas_sigi.crear_registro_nuevo_por_acta).
  5) Cada alta se comitea al instante (no se acumula nada en memoria para
     el final).
  6) Recorre TODAS las páginas siempre (no hay "pendientes" que agotar:
     se revisa cada fila de la tabla completa, dé o no dé alta).
  ...
  Si el script se corta a mitad de camino, lo ya dado de alta hasta ese
  punto queda guardado (commit inmediato por alta) -- se puede volver a
  correr tranquilo: las actas ya cargadas se saltean de nuevo.
...
async def _leer_datos_acta_detalle(page) -> dict:
    """Abre la pestaña 'Actas' y devuelve todo lo que hace falta leer de
    ahí: número de acta, patente, dirección y fecha/hora (ver
    reglas_sigi.extraer_*_de_texto). Todo se lee del mismo texto plano
    de la pestaña, en un solo round-trip por intento.

    OJO: el botón 'Actas' puede pintarse rápido (a veces ya viene
    seleccionado con aria-current="page"), pero el CONTENIDO de esa
    pestaña puede tardar un poco más en aparecer. Por eso reintentamos
    la lectura unas cuantas veces con una pausa corta, en vez de leer
    una sola vez apenas se resuelve el click.

    Devuelve {} si no se pudo abrir la pestaña o no apareció el número
    de acta (que es la señal de que el contenido ya cargó) tras todos
    los intentos.
    """
    ...
    for intento in range(1, intentos + 1):
        ...
        numero_acta = reglas_sigi.extraer_numero_acta_de_texto(texto)
        if numero_acta:
            return {
                "numero_acta": numero_acta,
                "patente": reglas_sigi.extraer_patente_de_texto(texto),
                "direccion": reglas_sigi.extraer_direccion_de_texto(texto),
                "fecha_hora": reglas_sigi.extraer_fecha_hora_de_texto(texto),
            }
        if intento < intentos:
...
async def _procesar_fila_con_reintento(
    ...
    """
    Procesa UNA fila completa (scroll, abrir detalle, leer acta/patente/
    dirección/fecha, decidir alta o salteo, volver) con reintentos si el
    DOM se la "come" debajo nuestro a mitad de camino.

    `actas_conocidas`: set de actas (normalizadas, ver
    reglas_sigi.normalizar_acta) que YA existen en la base -- se calcula
    una sola vez en ejecutar_alta con reglas_sigi.todas_las_actas_conocidas(db)
    y se va completando acá mismo con cada alta nueva (para no dar de
    alta dos veces la misma acta si aparece dos veces en la grilla
    durante la misma corrida).

    Por qué hace falta el reintento: después de cada 'Volver' del
    detalle, la tabla se vuelve a renderizar COMPLETA (React reemplaza
    los nodos, no los reutiliza). Si entre que ubicamos
    `fila = filas.nth(fila_idx)` y el momento en que efectivamente la
    usamos (scroll, click, lo que sea) ocurre un re-render, el handle
    que teníamos queda "not attached to the DOM". Con ~18.000 filas esto
    no es un caso raro, es matemática: tarde o temprano va a pasar, así
    que en vez de tratarlo como un error fatal lo tratamos como algo
    esperable y reintentable.
    ...
    salteada (no se cuenta como alta, sigue de largo) en vez de reventar
    todo el recorrido.

    Devuelve 1 si se dio de alta un registro nuevo en esta fila, 0 en
    cualquier otro caso (acta ya conocida, sin match legible, patente
    no leíble, o fila salteada por reintentos agotados).
    """
    for intento in range(1, max_intentos + 1):
        try:
            ...
            datos_acta = await _leer_datos_acta_detalle(page)
            numero_acta = datos_acta.get("numero_acta")
            acta_norm = reglas_sigi.normalizar_acta(numero_acta) if numero_acta else None

            alta_nueva = 0

            if not numero_acta:
                log("FILA", "  ❌ no se pudo leer el Nº de acta en el detalle -- salteo esta fila")

            elif acta_norm in actas_conocidas:
                log("FILA", f"  ↩ acta {numero_acta} ya existe en la base -- salteo")

            else:
                patente_leida = datos_acta.get("patente")
                if not patente_leida:
                    # patente es NOT NULL en el modelo: sin ella no se
                    # puede insertar el registro. Mejor saltear con un
                    # aviso claro que reventar el commit más abajo.
                    log("FILA", f"  ⚠️ acta {numero_acta} es nueva pero no se pudo leer la "
                                f"patente en el detalle (campo obligatorio) -- salteo esta "
                                f"fila, no se da de alta.")
                else:
                    ...
                    if commit:
                        reglas_sigi.crear_registro_nuevo_por_acta(
                            ...
                            direccion=datos_acta.get("direccion"),
                            fecha_hora=datos_acta.get("fecha_hora"),
                            estado_texto=estado_fila,
                            motivo_texto=motivo_texto,
                        )
                        db.commit()  # persistir YA, no esperar a que termine todo el recorrido
                        log("FILA", f"  ✅ ALTA acta {numero_acta} (expediente {expediente_fila}, "
                                    f"patente {patente_leida}) -- guardada en DB.")
                    else:
                        log("FILA", f"  (dry-run) ALTA acta {numero_acta} (expediente "
                                    f"{expediente_fila}, patente {patente_leida}, dirección "
                                    f"{datos_acta.get('direccion')}, fecha "
                                    f"{datos_acta.get('fecha_hora')}, estado {estado_fila}) "
                                    f"-- NO se graba.")
...
async def _procesar_todas_las_paginas(db: Session, page, actas_conocidas: set, commit: bool) -> int:
    """
    Recorre la tabla completa de SIGI, fila por fila, SIEMPRE hasta el
    final (no hay "pendientes" que agotar -- se revisa cada fila de la
    tabla completa, dé o no dé alta esa fila).
    ...
    Devuelve la cantidad total de altas nuevas.
    """
    ...
    while True:
        ...
        while True:
            ...
            altas += await _procesar_fila_con_reintento(
    ...
    log("FIN", f"Recorrido terminado: {altas} alta(s) nueva(s)")
    return altas
...
async def ejecutar_alta(db: Session, page, commit: bool = True) -> dict:
    ...
    Recorre TODA la tabla de SIGI una sola vez, fila por fila, comparando
    cada Nº de acta contra TODAS las actas que ya existen en la base (ver
    reglas_sigi.todas_las_actas_conocidas):

      - acta ya existe en la base -> se saltea la fila, no se toca nada.
      - acta nueva -> se da de alta un registro con expediente/acta/
        patente/dirección/fecha (leídos de SIGI) y estado_sigi según la
        columna ESTADO de la grilla. estado_semyt queda en 'Eliminada'.

    commit=True  -> cada alta se persiste al instante en la DB.
    commit=False -> dry-run: NO se toca la DB para nada, sólo se loguea
                    qué se habría dado de alta.
    """
    ...
    log("INICIO", f"{len(actas_conocidas)} acta(s) ya conocida(s) en la base "
    ...
    altas = await _procesar_todas_las_paginas(db, page, actas_conocidas, commit)
