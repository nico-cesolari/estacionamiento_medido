...
USO:
    cd backend
    python baja/solucionar_foto_url_mayor_a_menor.py                     # dry-run
    python baja/solucionar_foto_url_mayor_a_menor.py --commit
    python baja/solucionar_foto_url_mayor_a_menor.py --commit --limit 50  # probar con pocas
"""
import argparse
import asyncio
import sys
...
def _registros_con_foto_existente(db: Session, limite: Optional[int]):
    ...
    Orden: de acta más grande a más chica (las más nuevas primero).
    ...
    query = (
        ...
        .order_by(cast(models.Registro.acta, Integer).desc())
    )
    if limite:
